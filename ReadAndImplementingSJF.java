...
public class ReadAndImplementingSJF implements Runnable {
	...
	private static List<Integer> sortSJF = new ArrayList<>();
	...
	// constructor
	public ReadAndImplementingSJF(Scanner sc, String alg)
	{
		this.scan = sc; 
		this.alg_type = alg;
	}
	...
	public void run(){
		...
		SJFListBuilder sjfList_thread = new SJFListBuilder(process_cpu_cycle, process_io_cycle,num_proc, num_proc_cycle_max);
		
		try{
			sjfList_thread.start();
			sjfList_thread.join();
						
			sortSJF = sjfList_thread.SubsortSJF;
			Finalsorting  = getChunkList(sortSJF, 2);
			...
			for(int i=0; i<Finalsorting.size();i++){
			 ...
			 Collections.sort(process1, new Comparator<List<Integer>>(){
				 public int compare(List<Integer> arg0, List<Integer> arg1) {
			         return arg0.get(0).compareTo(arg1.get(0));
			     }
			});
			 
			 Collections.sort(process2, new Comparator<List<Integer>>(){
				 public int compare(List<Integer> arg0, List<Integer> arg1) {
			         return arg0.get(0).compareTo(arg1.get(0));
			     }
			});
			 
			 Collections.sort(process3, new Comparator<List<Integer>>(){
				 public int compare(List<Integer> arg0, List<Integer> arg1) {
			         return arg0.get(0).compareTo(arg1.get(0));
			     }
			});
			 
			 Collections.sort(process4, new Comparator<List<Integer>>(){
				 public int compare(List<Integer> arg0, List<Integer> arg1) {
			         return arg0.get(0).compareTo(arg1.get(0));
			     }
			});
	...
	SJFSchedulingAndLogic cpuScheduling_SJF_thread = new SJFSchedulingAndLogic(FinalList, num_proc, num_proc_cycle_max, 
	...
	try
	{
		...
		cpuScheduling_SJF_thread.start();
		cpuScheduling_SJF_thread.join();
			
		CPU_utilization = cpuScheduling_SJF_thread.cpu_utilization;
		throughput = cpuScheduling_SJF_thread.through;
		tar_time = cpuScheduling_SJF_thread.tar;
		wait_time = cpuScheduling_SJF_thread.wait;
		response_time = cpuScheduling_SJF_thread.response;
